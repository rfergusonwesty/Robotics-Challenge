/*
 ...
 Title:  magic_chassis_withIR_withHMC5883Compass
  ...
  else
  {
    ...
    //The robot input is set to "move" and nothing is in the way
      ...
      if(currentHeading < 270)      
      {
        ...
        digitalWrite(dir_a, HIGH);  // Right Forward
        digitalWrite(dir_b, HIGH);  // Left Forward

        analogWrite(pwm_a, 200);  //set motor a (right) to slower and b (left) to faster
        analogWrite(pwm_b, 255);
      ...
      if (currentHeading > 270)
      {
        ...
        analogWrite(pwm_a, 255);  //set motor a (right) to faster and b (left) to slower
